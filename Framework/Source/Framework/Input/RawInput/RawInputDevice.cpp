...
void RawInputDevice::Initialize()
{
    ...
    g_spKeyboard->Initialize(ApplicationChilled::GetApplication()->g_window->GetWndHandle());

    g_spMouse = std::make_shared<Mouse>();
    g_spMouse->Initialize(ApplicationChilled::GetApplication()->g_window->GetWndHandle());

    ApplicationChilled::GetApplication()->g_imGuiSystem->AddLog("INFO: RawInputDevice initialized.");
}
...
{
    g_spKeyboard->Finalize(ApplicationChilled::GetApplication()->g_window->GetWndHandle());
    g_spMouse->Finalize(ApplicationChilled::GetApplication()->g_window->GetWndHandle());
}
...
void RawInputDevice::ParseMessage(void* lparam)
{
    ...
    // マウスの場合
        ...
        g_spMouse->ParseMouseData(lprawinput->data.mouse, ApplicationChilled::GetApplication()->g_window->GetWndHandle());
