...
// でコンパイルします。/c
//
// Bounce - 文字 'a' が入力されるたびに新しいスレッドを作成します。
// 各スレッドは異なる色の文字をバウンスする。
// 画面に表示されます。すべてのスレッドは、文字 'Q' が入力されると終了する。
// 入力される。
//
...
// 関数 getrandom は、以下の間の乱数を返します。
// min と max は整数の範囲でなければならない．
#define getrandom( min, max ) (SHORT)((rand() % (int)(((max) + 1) - \
                               (min))) + (min))

int main(void);                    // スレッド1：メイン
void KbdFunc(void);                // キーボード入力、スレッドディスパッチ
void BounceProc(void* MyID);       // スレッド2～n：表示
void ClearScreen(void);            // 画面クリア
void ShutDown(void);               // プログラムのシャットダウン
void WriteTitle(int ThreadNum);    // タイトルバー情報を表示する
...
int main() // Thread One
{
    ...
    // ミュテックスを作成し、スレッドカウントをリセットします。
    hScreenMutex = CreateMutexW(NULL, FALSE, NULL);  // Cleared
    hRunMutex = CreateMutexW(NULL, TRUE, NULL);      // Set
    ThreadNr = 0;
    bTrails = FALSE;

    // キーボード入力待ちを開始し、スレッドのディスパッチや終了を行う。
    KbdFunc();

    // 全てのスレッドが終了しました。ハンドルをクリーンアップします。
