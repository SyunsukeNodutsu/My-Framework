...
static void MatrixMirrorZ(mfloat4x4& mat)
{
	// 回転のZミラーリング
	mat._13 *= -1;
	mat._23 *= -1;
...
std::shared_ptr<KdGLTFModel> GLTFLoader::KdLoadGLTFModel(const std::string& path)
{
	...
	//----------------------------------
	{
		...
		// マテリアル数だけ、配列確保
		destModel->m_materials.resize(model.materials.size());
		// 全マテリアルデータをコピー
		for (UINT matei = 0; matei < destModel->m_materials.size(); matei++)
		{
			const auto& srcMaterial = model.materials[matei];
			auto& destMaterial = destModel->m_materials[matei];

			// 名前
			destMaterial.m_name = srcMaterial.name;

			// フラグ系
			destMaterial.m_alphaMode = srcMaterial.alphaMode;
			destMaterial.m_alphaCutoff = (float)srcMaterial.alphaCutoff;
			destMaterial.m_doubleSided = srcMaterial.doubleSided;

			// 基本色
			destMaterial.m_baseColorTexture = GetTextureFilename(srcMaterial.pbrMetallicRoughness.baseColorTexture.index);
			if (srcMaterial.pbrMetallicRoughness.baseColorFactor.size() == 4)
			{
				destMaterial.m_baseColor.x = (float)srcMaterial.pbrMetallicRoughness.baseColorFactor[0];
				destMaterial.m_baseColor.y = (float)srcMaterial.pbrMetallicRoughness.baseColorFactor[1];
				destMaterial.m_baseColor.z = (float)srcMaterial.pbrMetallicRoughness.baseColorFactor[2];
				destMaterial.m_baseColor.w = (float)srcMaterial.pbrMetallicRoughness.baseColorFactor[3];
			}

			// 金属性、粗さ
			destMaterial.m_metallicRoughnessTexture = GetTextureFilename(srcMaterial.pbrMetallicRoughness.metallicRoughnessTexture.index);
			destMaterial.m_metallic = (float)srcMaterial.pbrMetallicRoughness.metallicFactor;
			destMaterial.m_roughness = (float)srcMaterial.pbrMetallicRoughness.roughnessFactor;

			// エミッシブ
			destMaterial.m_emissiveTexture = GetTextureFilename(srcMaterial.emissiveTexture.index);
			if (srcMaterial.emissiveFactor.size() == 3)
			{
				destMaterial.m_emissive.x = (float)srcMaterial.emissiveFactor[0];
				destMaterial.m_emissive.y = (float)srcMaterial.emissiveFactor[1];
				destMaterial.m_emissive.z = (float)srcMaterial.emissiveFactor[2];
			}

			// 法線マップ
			destMaterial.m_normalTexture = GetTextureFilename(srcMaterial.normalTexture.index);
			// オクルージョンマップ
			destMaterial.m_occlusionTexture = GetTextureFilename(srcMaterial.occlusionTexture.index);

		}

		// マテリアルがゼロの場合は、１つだけ作成しておく
		if (destModel->m_materials.size() == 0)destModel->m_materials.resize(1);
	...
	// 全ノードぶんメモリ確保
	destModel->m_nodes.resize(model.nodes.size());

	//----------------------------------
	// 全ノード 基本情報設定
	//----------------------------------
	for (UINT nodei = 0; nodei < destModel->m_nodes.size(); nodei++)
	{
		auto* destNode = &destModel->m_nodes[nodei];
		...
		// 名前
		destNode->m_name = model.nodes[nodei].name;

		// 子インデックス配列
		destNode->m_children = model.nodes[nodei].children;

		// 全ての子に、親設定
		for (auto&& idx : destNode->m_children)
		{
			destModel->m_nodes[idx].m_parent = nodei;
		}
		...
		if (model.nodes[nodei].rotation.size() != 0)
		{
			...
			mR = mfloat4x4::CreateFromQuaternion(q);
		}
		// 移動
		if (model.nodes[nodei].translation.size() != 0)
		{
			mT = mfloat4x4::CreateTranslation(
		...
		// 変換行列
		destNode->m_localTransform = mS * mR * mT;
		// Z軸ミラー
		MatrixMirrorZ(destNode->m_localTransform);

		// メッシュあり
		if (model.nodes[nodei].mesh >= 0)
		{
			// MeshフラグOn
			destNode->m_isMesh = true;
		}
	}
	...
	{
		destModel->m_rootNodeIndices.push_back(idx);
	}
	...
	{
		// 行列計算用 再帰関数
		std::function<void(KdGLTFNode*, const mfloat4x4*)> rec = [&rec, &destModel](KdGLTFNode* node, const mfloat4x4* parentMat)
		{
			if (parentMat) {
				node->m_worldTransform = node->m_localTransform * (*parentMat);
			}
			else {
			...
			{
				rec(&destModel->m_nodes[child], &node->m_worldTransform);
			}
		};

		// 親子関係から行列を作成
		for (int nodeIdx : destModel->m_rootNodeIndices)
		{
			rec(&destModel->m_nodes[nodeIdx], nullptr);
		}
	}
	...
	{
		// 配列確保
		destModel->m_boneNodeIndices = model.skins[0].joints;
		...
		for (UINT ji = 0; ji < model.skins[0].joints.size(); ji++)
		{
			...
			KdGLTFNode* boneNode = &destModel->m_nodes[nodeIdx];
			boneNode->m_boneNodeIndex = ji;

			// オフセット行列取得
			mfloat4x4 invBindMat;
			for (int mati = 0; mati < 16; mati++)
			{
				(&invBindMat._11)[mati] = ibmGetter.GetValue_Float(ji * 16 + mati);
			}
			MatrixMirrorZ(invBindMat);
			boneNode->m_inverseBindMatrix = invBindMat;
			// 変換行列へ変換
			boneNode->m_worldTransform = invBindMat.Invert();
		}

		// ボーンLocalMat算出
		for (int nodeIdx : destModel->m_boneNodeIndices)
		{
			KdGLTFNode* boneNode = &destModel->m_nodes[nodeIdx];

			if (boneNode->m_parent >= 0)
			{
				boneNode->m_localTransform = boneNode->m_worldTransform * destModel->m_nodes[boneNode->m_parent].m_inverseBindMatrix;
			}
			else
			{
				boneNode->m_localTransform = boneNode->m_worldTransform;
			}
		}
	}
	...
	for (UINT nodei = 0; nodei < destModel->m_nodes.size(); nodei++)
	{
		...
		for (size_t pri = 0; pri < model.meshes[msi].primitives.size(); pri++)
		{
			...
			// 頂点バッファ
			{
				...
				// 座標
				{
					...
					for (UINT vi = 0; vi < posGetter.GetAccessor()->count; vi++) {
						...
						ver.m_pos.x = posGetter.GetValue_Float(vi * 3 + 0);
						ver.m_pos.y = posGetter.GetValue_Float(vi * 3 + 1);
						ver.m_pos.z = posGetter.GetValue_Float(vi * 3 + 2) * -1;
					}
				}
				...
				if (srcPrimitive.attributes.count("NORMAL") > 0)
				{
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++) {
						auto& nor = destPrimitive->Vertices[vi].m_normal;
				...
				if (srcPrimitive.attributes.count("TANGENT") > 0)
				{
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++) {
						auto& tan = destPrimitive->Vertices[vi].m_tangent;
						tan.x = tangentGetter.GetValue_Float(vi * 3 + 0);
				...
				if (srcPrimitive.attributes.count("TEXCOORD_0") > 0)
				{
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++) {
						auto& uv = destPrimitive->Vertices[vi].m_uv;
				...
				if (srcPrimitive.attributes.count("COLOR_0") > 0)
				{
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++)
					{
						...
						destPrimitive->Vertices[vi].m_color = color.RGBA().v;
					}
				}
				...
				{
					destNode->m_mesh.m_isSkinMesh = true;
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++)
					{
						...
						auto& skinIndex = destPrimitive->Vertices[vi].m_skinIndexList;
				...
				if (srcPrimitive.attributes.count("WEIGHTS_0") > 0)
				{
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++)
					{
						auto& skinWei = destPrimitive->Vertices[vi].m_skinWeightList;
			...
			// インデックスバッファ
			{
				...
				for (UINT di = 0; di < destPrimitive->Faces.size(); di++)
				{
					...
					destPrimitive->Faces[di].m_index[0] = (UINT)indexGetter.GetValue_Int(di * 3 + 0);
					destPrimitive->Faces[di].m_index[2] = (UINT)indexGetter.GetValue_Int(di * 3 + 1);
					destPrimitive->Faces[di].m_index[1] = (UINT)indexGetter.GetValue_Int(di * 3 + 2);
				}
			}
		...
		// マテリアルの最大数ぶんサブセット作成
		destNode->m_mesh.m_subsets.resize(tempPrimitives.size());
		for (UINT pi = 0; pi < tempPrimitives.size(); pi++)
		{
			// マテリアル番号
			destNode->m_mesh.m_subsets[pi].m_materialNo = tempPrimitives[pi]->MaterialNo;
		}
		...
		for (UINT pi = 0; pi < tempPrimitives.size(); pi++)
		{
			...
			if (prim->Faces.size() >= 1) {
				...
				for (UINT fi = 0; fi < prim->Faces.size(); fi++) {
					destNode->m_mesh.m_faces[st + fi].m_index[0] = prim->Faces[fi].m_index[0] + currentVertexIdx;
					destNode->m_mesh.m_faces[st + fi].m_index[1] = prim->Faces[fi].m_index[1] + currentVertexIdx;
					destNode->m_mesh.m_faces[st + fi].m_index[2] = prim->Faces[fi].m_index[2] + currentVertexIdx;
				}
			}
		...
		{
			UINT offset = 0;
			for (UINT pi = 0; pi < destNode->m_mesh.m_subsets.size(); pi++)
			{
				destNode->m_mesh.m_subsets[pi].m_faceStart = offset;	// 開始Index
	...
	for (UINT ani = 0; ani < model.animations.size(); ani++)
	{
		...
		destModel->m_animations.push_back(animation);
		...
		tempNodes.resize(destModel->m_nodes.size());
